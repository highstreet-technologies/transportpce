                    ...
                    if found == False or replaceMultiple:
                        ...
                        if not isComment:
                            ...
                            for matchNum, match in enumerate(matches, 1):
                                f = match.group(1)
                                # detect self-closing tag (e.g. <circuit-pack-name />)
                                # the regex capture stops at the first space, so the
                                # trailing '/' is not part of 'f'; inspect the raw tag.
                                tagEnd = line.find('>', match.end())
                                isAutoclose = tagEnd > 0 and line[tagEnd-1] == '/'
                                # end tag detected
                                if f.startswith("/"):
                ...
                for line in src_file:
                    if found == False or replaceMultiple:
                        ...
                        if not isComment:
                            ...
                            for matchNum, match in enumerate(matches, 1):
                                f = match.group(1)
                                # detect self-closing tag (see removeXmlEntry)
                                tagEnd = line.find('>', match.end())
                                isAutoclose = tagEnd > 0 and line[tagEnd-1] == '/'
                                # end tag detected
                                if f.startswith("/"):
