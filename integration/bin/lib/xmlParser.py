...
class OpenRoadmXmlParser:
    ...
    def __init__(self, basePath:str, roadmTemplateFile:str=None, xpdrTemplateFile:str=None, outputPath:str=None) -> None:
        self.basePath = basePath.rstrip('/')
        self.roadmTemplateFile = '{}/{}'.format(self.basePath,'integration/demo-standalone/conf/oper-ROADMA.xml') if roadmTemplateFile is None else roadmTemplateFile.rstrip('/')
        self.xpdrTemplateFile = '{}/{}'.format(self.basePath,'integration/demo-standalone/conf/oper-XPDRA.xml') if xpdrTemplateFile is None else xpdrTemplateFile.rstrip('/')
    ...
    def create_info(self, device, ns, device_name, deg, deviceType, node_num):
    ...
    def create_degree(self, sel, ns,deg):
        degree_list=[]
    ...
    def create_srgs(self, srg,ns, srg_no, cp_list):
        srg_list=[]
        srg_cp_name_list=[]
    ...
    def cp_copies(self, roadm,ns):
        ...
        roadm=self.del_circuit_packs(roadm,ns)
        return roadm, parent_cp, eth_cp, osc_cp, srg_cp

    def create_circuit_pack(self, cp_list,eth_cp,osc_cp, parent_cp, ns):
        cp_list_body=[]
        for c in cp_list:
            if 'ETH-PLUG' in c:
    ...
    def create_srg_deg_ports(self, srg_ports, num, ns):
        ad_srg_ports=[]
        for p in srg_ports:
    ...
    def create_srg_cp(self, srg_list, srg_cp_body, ns):
        srg_cp_body_list=[]
        for n in srg_list:
    ...
    def create_interface_copies(self, roadm, ns):
    ...
    def del_circuit_packs(self, roadm, ns):
    ...
    def create_data_models(self, dev_name:str, deg_number, device_type:str, node_num:int):
        ...
        cp_name_list=[]
        interface_list =[]
        roadm_root=self.create_info(roadm_root, ns, dev_name, str(deg_number),device_type, node_num)
        ...
        degrees=self.create_degree(sel,ns,deg_number)
        ...
        roadm_root,parent_cp, eth_cp, osc_cp, srg_cp= self.cp_copies(roadm_root, ns)
        cp_list=self.create_circuit_pack(cp_name_list,eth_cp,osc_cp, parent_cp, ns)
        for elem in cp_list:
        ...
        srgs, srg_cp_name_list=self.create_srgs(srg, ns, deg_number, cp_name_list)
        ...
        deg_port= self.create_srg_deg_ports(srg_ports,deg_number, ns)
        for d in deg_port:
            srg_cp.append(d)
        srg_cp_body_list= self.create_srg_cp(srg_list=srg_cp_name_list, srg_cp_body=srg_cp, ns=ns)
        for s in srg_cp_body_list:
            roadm_root.append(s)
        
        # create the interface bodies and append
        ge_interface, ots_interface, oms_interface=self.create_interface_copies(roadm_root, ns)
        for i in range(1,5):
    ...
    def create_xpdr_data_models(self, dev_name:str, device_type:str, node_num:int):
        ET.register_namespace('', 'http://org/openroadm/device')
        ns ={'device':'http://org/openroadm/device'}
        with open(self.xpdrTemplateFile, 'r') as x:
            xmlxpdr=x.read()
            x.close()
        xpdr_root=ET.fromstring(xmlxpdr)
        xpdr_root=self.create_info(xpdr_root, ns, dev_name, None,device_type, node_num)
